...
//            navigationCallback.popBackStack()   // TODO replace with Settings Screen
    ...
    private fun showSearchToolbar() {
        with (binding) {
            ...
            val searchAnimator = ObjectAnimator.ofFloat(toolbarSearch, View.X, View.Y, searchPath).apply {
                duration = 400
                start()
            }
            val toolbarAnimator = ObjectAnimator.ofFloat(toolbar, View.X, View.Y, toolbarPath).apply {
                duration = 400
                start()
            }
        }
