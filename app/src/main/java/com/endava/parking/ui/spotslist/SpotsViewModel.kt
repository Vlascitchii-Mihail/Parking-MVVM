...
//    private var _parkingLotInstance: MutableLiveData<List<Spot>> = MutableLiveData()
//    val parkingLotInstance: LiveData<List<Spot>> get() = _parkingLotInstance

    private var _parkingSpots: MutableLiveData<List<Spot>> = MutableLiveData()
    val parkingSpots: LiveData<List<Spot>> get() = _parkingSpots
//    private var parkingSpots: List<Spot> = emptyList()
...
//    lateinit var lotDescription: List<String>
    ...
    fun getParkingSpots(parkingName: String, levelName: String) {
        viewModelScope.launch {
            try {
                val response = parkingRepository.getParkingSpots(parkingName, levelName)
                //returns nullable type ParkingLot?
                val body = response.body()
                if (response.isSuccessful && body != null) {
                    //without nullable ParkingLot
                    body.let { parkingSpots ->
                        _parkingSpots.value = parkingSpots
                    }
